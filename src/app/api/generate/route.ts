...
**ТЕКСЕРІЛДІ: ______________________________ № _____ ОМ**
...
| **Сабақ түрі:** | ${lessonType} |
| **Оқу мақсаттары:** | [Бағдарлама коды, мысалы: ${grade.padStart(1, "")}.X.X.X] [Нақты оқу мақсатының мәтіні — оқушы нені біледі / орындай алады] |
...
МАҢЫЗДЫ ЕРЕЖЕЛЕР:
- Барлық кестелер мен бөлімдерді ТОЛЫҚ аяқта — ешбір жол немесе өріс жартылай қалмасын.
...
export async function POST(request: Request) {
  try {
    const { subject, grade, topic, lessonType, duration, assessmentType, specialNeeds, teacherName } =
      await request.json();
